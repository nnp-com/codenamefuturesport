...
import { Sport, TeamMember } from '../types';
...
export const performRound = (
  ...
  for (let i = 0; i < 6; i++) {
    ...
    events.push(`${attacker1.name} (${attacker1.sport}) ${result1.action} against ${defender1.name} (${defender1.sport}) for ${result1.points} points`);
    ...
    events.push(`${attacker2.name} (${attacker2.sport}) ${result2.action} against ${defender2.name} (${defender2.sport}) for ${result2.points} points`);
  }

  return { player1Score, player2Score, events };
};

// Function to perform a full match (12 rounds)
export const performMatch = (
  player1Team: TeamMember[],
  player2Team: TeamMember[]
): { player1TotalScore: number; player2TotalScore: number; roundResults: Array<{ round: number; player1Score: number; player2Score: number; events: string[] }> } => {
  ...
  for (let round = 1; round <= 12; round++) {
    ...
    roundResults.push({ round, player1Score, player2Score, events });
  }

  return { player1TotalScore, player2TotalScore, roundResults };
};